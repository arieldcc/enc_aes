...
def get_encryption_data():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT nama_file_asli, ekstensi_file, nama_file_enc, nama_file_bin, key_enc, ukuran_file, waktu_eksekusi, memori_digunakan FROM data_enkripsi')
...
def get_decryption_data():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('''SELECT nama_file_enc, ekstensi_file, file_desk, ukuran_file_desk, waktu_eksekusi_desk, memori_digunakan_desk 
                      FROM data_enkripsi''')
    data = cursor.fetchall()
    conn.close()
    return data
...
def decrypt():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute('SELECT key_enc, ekstensi_file, file_desk FROM data_enkripsi WHERE nama_file_enc = ?', (enc_filename,))
        row = cursor.fetchone()
